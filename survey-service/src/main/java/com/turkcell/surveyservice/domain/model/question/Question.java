...
import com.turkcell.surveyservice.domain.model.survey.Option;
...
public class Question {
    ...
    public void removeOption(String optionText) {
        boolean removed = options.removeIf(option -> option.text().trim().equalsIgnoreCase(optionText));
        if (!removed) {
    ...
    public void checkDuplicateOption(String optionText) {
        for (Option option : options) {
            if (option.text().trim().equalsIgnoreCase(optionText)) {
